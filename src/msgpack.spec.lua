...
return function()
	...
	describe("decode", function()
		...
		it("can decode fixstr value", function()
			...
			).to.equal("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
		end)
		...
		it("can decode str 32 value", function()
			...
			expect(msgpack.decode("\xDB\x00\x00\x00\x0B\x68\x65\x6C\x6C\x6F\x20\x77\x6F\x72\x6C\x64")).to.equal(
				"hello world"
			)
		end)
		...
		it("can decode map 32 value", function()
			...
			local filledMap = msgpack.decode(
				"\xDF\x00\x00\x00\x02\xA5\x68\x65\x6C\x6C\x6F\xA5\x77\x6F\x72\x6C\x64\x7B\xC3"
			)
			expect(#filledMap).to.equal(0)
			expect(next(filledMap)).to.be.ok()
			expect(filledMap["hello"]).to.equal("world")
	...
	describe("encode", function()
		...
		it("can encode Extension value", function()
			...
			expect(hex(msgpack.encode(msgpack.Extension.new(123, "wxyzwxyz")))).to.equal(
				"D7 7B 77 78 79 7A 77 78 79 7A"
			)
			expect(hex(msgpack.encode(msgpack.Extension.new(123, "wxyzwxyzwxyzwxyz")))).to.equal(
